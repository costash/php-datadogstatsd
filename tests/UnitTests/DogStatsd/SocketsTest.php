...
#    }
    ...
    public function testTelemetryAllDataType()
    {
        ...
        $dog->timing('test', 21);
        $this->assertSameTelemetry('test:21|ms', $this->getSocketSpy()->argsFromSocketSendtoCalls[0][1]);

        $dog->gauge('test', 21);
        $this->assertSameTelemetry('test:21|g', $this->getSocketSpy()->argsFromSocketSendtoCalls[1][1], "", array("bytes_sent" => 675, "packets_sent" => 1));

        $dog->histogram('test', 21);
        $this->assertSameTelemetry('test:21|h', $this->getSocketSpy()->argsFromSocketSendtoCalls[2][1], "", array("bytes_sent" => 676, "packets_sent" => 1));

        $dog->distribution('test', 21);
        $this->assertSameTelemetry('test:21|d', $this->getSocketSpy()->argsFromSocketSendtoCalls[3][1], "", array("bytes_sent" => 676, "packets_sent" => 1));

        $dog->set('test', 21);
        $this->assertSameTelemetry('test:21|s', $this->getSocketSpy()->argsFromSocketSendtoCalls[4][1], "", array("bytes_sent" => 676, "packets_sent" => 1));

        $dog->increment('test');
        $this->assertSameTelemetry('test:1|c', $this->getSocketSpy()->argsFromSocketSendtoCalls[5][1], "", array("bytes_sent" => 676, "packets_sent" => 1));

        $dog->decrement('test');
        $this->assertSameTelemetry('test:-1|c', $this->getSocketSpy()->argsFromSocketSendtoCalls[6][1], "", array("bytes_sent" => 675, "packets_sent" => 1));

        $dog->event('ev', array('text' => 'text'));
        $this->assertSameTelemetry('_e{2,4}:ev|text', $this->getSocketSpy()->argsFromSocketSendtoCalls[7][1], "", array("bytes_sent" => 676, "packets_sent" => 1, "metrics" => 0, "events" => 1));

        $dog->service_check('sc', 0);
        $this->assertSameTelemetry('_sc|sc|0', $this->getSocketSpy()->argsFromSocketSendtoCalls[8][1], "", array("bytes_sent" => 682, "packets_sent" => 1, "metrics" => 0, "service_checks" => 1));

        # force flush to get the telemetry about the last message sent
        $dog->flush("");
        $this->assertSameTelemetry('', $this->getSocketSpy()->argsFromSocketSendtoCalls[9][1], "", array("bytes_sent" => 675, "packets_sent" => 1, "metrics" => 0));
    }
    ...
    public function testTelemetryNetworkError()
    {
        ...
        $dog->gauge('test', 22);
        $this->assertSameTelemetry('test:22|g', $this->getSocketSpy()->argsFromSocketSendtoCalls[0][1], "", array("metrics" => 3, "bytes_dropped" => 1351, "packets_dropped" => 2));

        # force flush to get the telemetry about the last message sent
        $dog->flush("");
        $this->assertSameTelemetry('', $this->getSocketSpy()->argsFromSocketSendtoCalls[1][1], "", array("bytes_sent" => 677, "packets_sent" => 1, "metrics" => 0));
    }
